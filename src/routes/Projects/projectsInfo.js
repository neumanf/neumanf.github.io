export const projectsInfo = [
  {
    title: "Mally",
    description:
      "A collection of web services such as URL shorteners, pastebins, and many more.",
    badges: ["TypeScript", "React", "NextJS", "NestJS", "MySQL"],
    features: [
      "URL Shortener and Pastebin services",
      "Authentication with JWT",
      "User dashboard",
      "CI/CD",
      "Tests",
    ],
    liveLink: "https://mally.neumanf.com",
    sourceLink: "https://github.com/neumanf/mally",
    pageLink: "/projects/mally",
    screenshots: [
      {
        link: "/assets/images/mally.png",
        caption: "Homepage",
      },
      {
        link: "/assets/images/mally_url_shortener.png",
        caption: "URL Shortener",
      },
      {
        link: "/assets/images/mally_pastebin.png",
        caption: "Pastebin",
      },
      {
        link: "/assets/images/mally_dashboard.png",
        caption: "Dashboard",
      },
    ],
  },
  {
    title: "Vagas Tech RN Bot",
    description: "Manages the Vagas Tech RN channel on Telegram.",
    badges: ["TypeScript", "Express", "grammY", "MySQL"],
    liveLink: "https://t.me/s/vagastechrn",
    sourceLink: "https://github.com/neumanf/vagastechrnbot",
    pageLink: "/projects/vagastechrnbot",
    screenshots: [
      {
        link: "/assets/images/vagastechrnbot.png",
        caption: "Channel",
      },
    ],
  },
  {
    title: "Clean E-commerce",
    description:
      "E-commerce/E-shop website capable of list, filter and order products, as well as notify users when a product is unavailable.",
    badges: ["TypeScript", "React", "NextJS", "NestJS", "PostgreSQL"],
    sourceLink: "https://github.com/neumanf/clean-ecommerce",
    pageLink: "/projects/clean-ecommerce",
    screenshots: [
      {
        link: "/assets/images/clean-ecommerce.png",
        caption: "Homepage",
      },
    ],
  },
  {
    title: "Image Deblur Tool",
    description: "Restores blurred images using the Wiener filter.",
    badges: ["OpenCV", "Python", "FastAPI", "React"],
    liveLink: "https://image-deblur-tool.vercel.app",
    sourceLink: "https://github.com/neumanf/image-deblur-tool",
    pageLink: "/projects/image-deblur-tool",
    screenshots: [
      {
        link: "/assets/images/image-deblur-tool.png",
        caption: "Homepage",
      },
    ],
  },
  {
    title: "Unboreme",
    description:
      "Movies and TV series recommendation website. Choose one or more categories and it will recommend you something based on that.",
    badges: ["TypeScript", "Svelte", "AdonisJS", "TailwindCSS"],
    liveLink: "https://unboreme.herokuapp.com",
    sourceLink: "https://github.com/neumanf/unboreme",
    pageLink: "/projects/unboreme",
    screenshots: [
      {
        link: "/assets/images/unboreme.png",
        caption: "Homepage",
      },
    ],
  },
  {
    title: "PDI",
    description:
      "Blog including projects from Digital Image Processing classes.",
    badges: ["C++", "OpenCV"],
    liveLink: "https://neumanf.github.io/pdi",
    sourceLink: "https://github.com/neumanf/pdi",
    pageLink: "/projects/pdi",
    screenshots: [
      {
        link: "/assets/images/pdi.png",
        caption: "Homepage",
      },
    ],
  },
];
